...
__device__ Vec3_d vec3d_from_double_device(double f) {
  return (Vec3_d){.x = f, .y = f, .z = f};
}

__device__ Vec3_d vec3d_reflect_device(Vec3_d v, Vec3_d n) {
  Vec3_d _a = vec3d_mul_device(
      vec3d_mul_device(vec3d_from_double_device(2.0),
                       vec3d_from_double_device(vec3d_dot_device(v, n))),
      n);
  return vec3d_sub_device(v, _a);
}
...
__device__ double vec3d_cosine_sim_device(Vec3_d v1, Vec3_d v2) {
  return vec3d_dot_device(v1, v2) /
         (vec3d_length_device(v1) * vec3d_length_device(v2));
}

// Kernel for adding arrays of Vec3_d
__global__ void vec3d_add_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                 int n) {
...
__global__ void vec3d_sub_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                 int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_mul_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                 int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_div_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                 int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_dot_kernel(Vec3_d *v1, Vec3_d *v2, double *result,
                                 int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_cosine_similarity_kernel(Vec3_d *v1, Vec3_d *v2,
                                               double *result, int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_cross_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                   int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void vec3d_reflect_kernel(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                                     int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    result[idx] = vec3d_reflect_device(v1[idx], v2[idx]);
  }
}
...
void vec3d_add_cuda(Vec3_d *v1, Vec3_d *v2, Vec3_d *result, int batch_size) {
  ...
  vec3d_add_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                       batch_size);
  ...
  if (cudaMemcpy(result, d_c, vec_memsize, cudaMemcpyDeviceToHost) !=
      cudaSuccess) {
...
void vec3d_sub_cuda(Vec3_d *v1, Vec3_d *v2, Vec3_d *result, int batch_size) {
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_sub_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                       batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
...
void vec3d_dot_cuda(Vec3_d *v1, Vec3_d *v2, double *result, int batch_size) {
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_dot_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                       batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
  ...
  if (cudaMemcpy(result, d_c, double_memsize, cudaMemcpyDeviceToHost) !=
      cudaSuccess) {
    fprintf(stderr, "CUDA memory copy to host failed\n");
  }
...
void vec3d_mul_cuda(Vec3_d *v1, Vec3_d *v2, Vec3_d *result, int batch_size) {
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_mul_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                       batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
...
void vec3d_cross_cuda(Vec3_d *v1, Vec3_d *v2, Vec3_d *result, int batch_size) {
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_cross_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                         batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
...
void vec3d_cosine_similarity_cuda(Vec3_d *v1, Vec3_d *v2, double *result,
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_cosine_similarity_kernel<<<blocksPerGrid, threadsPerBlock>>>(
      d_a, d_b, d_c, batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
            cudaGetErrorString(kernelError));
...
void vec3d_reflect_cuda(Vec3_d *v1, Vec3_d *v2, Vec3_d *result,
                        int batch_size) {
  size_t vec_memsize = batch_size * sizeof(Vec3_d);
  Vec3_d *d_a, *d_b, *d_c;
  if (cudaMalloc(&d_a, vec_memsize) != cudaSuccess ||
  ...
  int threadsPerBlock = 256;
  int blocksPerGrid = (batch_size + threadsPerBlock - 1) / threadsPerBlock;
  vec3d_reflect_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c,
                                                           batch_size);
  cudaError_t kernelError = cudaGetLastError();
  if (kernelError != cudaSuccess) {
    fprintf(stderr, "Kernel launch error: %s\n",
